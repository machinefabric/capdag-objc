...
- (instancetype)initWithConfig:(CSFabricRegistryConfig *)config {
    ...
    if (self) {
        ...
        // Manifest pin from the BAKED constant (CSFabricManifestVersion.m,
        // regenerated by scripts/lib/config.sh from fabric/manifest-version.txt)
        // — the ObjC analogue of Rust's compile-time FABRIC_MANIFEST_VERSION.
        // We do NOT read MFR_FABRIC_MANIFEST_VERSION from the process
        // environment and do NOT fall back to v0: a process that happened to
        // lack the env var (e.g. the app's UI process) silently degraded to
        // legacy flat-path lookups that 404 every versioned cap/media def. A
        // value < 1 is a hard failure here, mirroring build.rs's refusal to
        // build below v1.
        ...
        _manifestVersion = CSBakedFabricManifestVersion;
