...
/// One file a scaffold writes: its path inside the project, its contents, and
/// whether the host must be able to exec it.
public struct StubFile: Sendable {
...
///
/// `entry` is the executable the host launches, placeholder-substituted like
/// any other rendered path. `build` holds the commands that turn the sources
/// into it — empty for interpreted languages, which have nothing to build.
public struct StubLanguage: Sendable {
    ...
    public let runtimeRegistry: String
    public let runtimeInstall: String
    public let files: [StubFile]
}

/// The token replaced by the project name in every file's CONTENTS and in its
/// destination path and entry.
public let stubPlaceholder = #"__CARTRIDGE_NAME__"#

public let stubContractVersion = 1

/// Every language `capdag new` can scaffold, in contract order.
public let stubLanguages: [StubLanguage] = [
    StubLanguage(
        id: #"python"#,
        flag: #"--python"#,
        display: #"Python"#,
        entry: #"cartridge.py"#,
        build: [],
        runtimePackage: #"capdag"#,
        runtimeRegistry: #"PyPI"#,
        runtimeInstall: #"pip install capdag"#,
        files: [
            StubFile(
                dest: #"cartridge.py"#,
                contents: #"""
...
"""#,
    ...
    StubLanguage(
        ...
        runtimeInstall: #"cargo build (the dependency is declared in Cargo.toml)"#,
        files: [
            StubFile(
                dest: #"src/main.rs"#,
                contents: ##"""
...
capdag = { git = "https://github.com/machinefabric/capdag-rs", tag = "v1.649.0" }
ciborium = "0.2"
serde_json = "1.0"
...
"""#,
            ...
            StubFile(
                ...
                contents: #"""
# capdag's build script bakes two versions into the crate and treats both as
...
require github.com/machinefabric/capdag-go v1.343.0
...
"""#,
    ...
    StubLanguage(
        ...
        flag: #"--swift"#,
        display: #"Swift"#,
        entry: #".build/release/__CARTRIDGE_NAME__"#,
        build: [#"swift build -c release"#],
        runtimePackage: #"capdag-objc"#,
        runtimeRegistry: #"git tag"#,
        runtimeInstall: #"swift build (the dependency is declared in Package.swift)"#,
        files: [
            StubFile(
...
let package = Package(
    ...
    dependencies: [
        .package(url: "https://github.com/machinefabric/capdag-objc.git", from: "1.403.0"),
